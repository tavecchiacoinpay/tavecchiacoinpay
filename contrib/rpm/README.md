...
The RPM spec file provided here is for TavecchiacoinPay-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://tavecchiacoinpay.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[TavecchiacoinPay git repository](https://github.com/tavecchiacoinpay/tavecchiacoinpay) but are not part of
...
Currently only `Source10` is used, for the example `tavecchiacoinpay.conf` file.
...
in the tavecchiacoinpay git repository. At present only an SVG version of the TavecchiacoinPay
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the tavecchiacoinpay
...
needed when 0.12.1 is released, a proper fix is already in the TavecchiacoinPay git
master branch.
...
idea to build TavecchiacoinPay Core against a static version of that library compiled
...
The `tavecchiacoinpay.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `tavecchiacoinpay-server` package. At this point, most RPM based
...
The `tavecchiacoinpay-server` package should have SELinux support. How to properly do
...
TavecchiacoinPay-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/tavecchiacoinpay/). He has been
packaging TavecchiacoinPay for Fedora at least since 2012.
...
4. I renamed the `tavecchiacoinpay` package that contains the Qt GUI to `tavecchiacoinpay-core` as
that appears to be how the general population refers to it, in contrast to
`tavecchiacoinpay-xt` or `tavecchiacoinpay-classic`. I wanted to make sure the general population
