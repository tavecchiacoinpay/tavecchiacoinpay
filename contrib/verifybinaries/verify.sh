...
###   tavecchiacoinpaycore.org and tavecchiacoinpay.org and compares them.
...
WORKINGDIR="/tmp/tavecchiacoinpay_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://tavecchiacoinpaycore.org"
HOST2="https://tavecchiacoinpay.org"
BASEDIR="/bin/"
VERSIONPREFIX="tavecchiacoinpay-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tavecchiacoinpay-',
...
if [ $? -ne 0 ]; then
   echo "tavecchiacoinpay.org failed to provide signature file, but tavecchiacoinpaycore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "tavecchiacoinpay.org and tavecchiacoinpaycore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the TavecchiacoinPay Core binary release signing key installed?"
   fi
