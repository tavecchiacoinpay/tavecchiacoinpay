#!/bin/bash
# Copyright (c) 2013 The TavecchiacoinPay Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete TavecchiacoinPay database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no TavecchiacoinPay datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old TavecchiacoinPay datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected TavecchiacoinPay 0.7 datadir."
    ;;
  3)
    echo "Detected TavecchiacoinPay pre-0.8 datadir."
    ;;
  4)
    echo "Detected TavecchiacoinPay 0.8 datadir."
    ;;
esac
