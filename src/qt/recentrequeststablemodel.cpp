    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return TavecchiacoinPayUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, TavecchiacoinPayUnits::separatorNever);
            else
                return TavecchiacoinPayUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+TavecchiacoinPayUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
