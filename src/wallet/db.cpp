...
//! so tavecchiacoinpay should never create different databases with the same fileid, but
...
bool CDBEnv::Open(const fs::path& pathIn, bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of tavecchiacoinpay may be using it.\n", strPath);
        return false;
    }
